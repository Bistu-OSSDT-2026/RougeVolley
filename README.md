...
- [] WASD / 方向键 移动，鼠标瞄准射击
- [] 6 个不同形状房间随机拼接（3×2 网格）
- [] 至少 1 种敌人（Undead），含巡逻与追击 AI
- [] 5 种升级选项，清空房间敌人后三选一
- [] 属性叠加可感知（射速、弹丸数、伤害、弹速变化）
- [] 玩家死亡 → GameOver 界面，可重新开始或返回主菜单
- [] 开始界面、暂停菜单（含属性/升级展示）
- [] ESC 暂停/继续
- [] 像素精灵渲染（玩家/敌人/子弹/瓦片）
- [] 种子确定性生成（可重放）
...
---  
## 贡献者：  
- **mousquetaire123（架构/主程与测试）**：FXGL 入口、ECS 核心、系统调度、集成、打包、整体测试  
- **MemoryDeep（玩法逻辑）**：战斗系统、子弹、碰撞、属性修改  
- **BanqiD（地牢与敌人）**：房间模板制作、房间生成器、敌人 AI  
- **klose12313（UI/数据）**：UI 界面、JSON 配置、升级管理器  
- **gugugugugu-JH（资源）**：AI 生成像素美术、音效  
